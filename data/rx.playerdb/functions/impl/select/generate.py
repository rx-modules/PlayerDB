...
TREE = 'execute if score $bit rx.temp matches {low}..{high} run function rx.playerdb:impl/select/bit{num}/{low}_{high}'  # noqa: E501

LEAF = (
    'execute if score $bit rx.temp matches @ if data storage rx:global playerdb.players[{selected:1b, bits:{b%:@b}}] store result score $size rx.temp run data modify storage rx:global playerdb.players[{selected:1b, bits:{b%:@b}}].bits.select set value 1b'  # noqa: E501
)


def make_file(path, content):
    global file_count, cmd_count
